...
<div class="login-box">
  ...
  <div class="login-box-body">
    ...
    <form class="form-horizontal" role="form" method="POST" action="{{ route('password.reset.confirm') }}">
                        {{ csrf_field() }}
                        ...
                        <div class="form-group{{ $errors->has('email') ? ' has-error' : '' }}">
                            ...
                            <div class="col-md-6">
                                <input id="email" type="email"  class="form-control" name="constacl_single_users_email" value="{{ $email }}" readonly="" required autofocus="">

                                @if ($errors->has('constacl_single_users_email'))
                                    <span class="help-block">
                                        <strong>{{ $errors->first('constacl_single_users_email') }}</strong>
                                    </span>
                                @endif
                            </div>
                        </div>

                        <div class="form-group{{ $errors->has('constacl_single_users_password') ? ' has-error' : '' }}">
                            ...
                            <div class="col-md-6">
                                ...
                                @if ($errors->has('password'))
                                    <span class="help-block">
                                        <strong>{{ $errors->first('constacl_single_users_password') }}</strong>
                                    </span>
                                @endif
                            </div>
                        </div>
